...
async def monitor_returns():
    ...
    try:
        # 초기 가격 저장용 변수
        symbol = 'MOODENG/USDT:USDT'  # 선물 계약 심볼
        ...
        while True:
            try:
                ...
                # 5초 이전 데이터 제거
                while price_history and (current_time - price_history[0][0]).total_seconds() > 5:
                ...
                print(f"\r[{current_time.strftime('%H:%M:%S')}] {symbol} | "
                      ...
                      f"5초수익률: {returns:+.4f}%", end='', flush=True)
